...
The program must accept two string values S1 and S2 representing two sentences as the input.
The program must print YES if the two sentences are shadows of each other. 
...
Input:
four ten history tent was damaged
Output:
YES
Explanation:
...
if len(a) != len(b):
    print("NO")
elif any(len(a[c]) != len(b[c]) for c in range(len(a))):
    print("NO")
else:
    d = 1
    for c in range(len(a)):
        if any(e in b[c] for e in a[c]):
            print("NO")
            d = 0
            break
    if d:
