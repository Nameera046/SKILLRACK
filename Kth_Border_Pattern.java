...
// Then the program must replace the asterisks in the Kth border of the grid with the hash symbols(#). 
// Finally, the program must print the N'N grid as the output.
...
public class boardpattern {
	...
	public static void main(String[] args) {
		...
		for(int i=1;i<=n;i++)
		{
			for(int j=1;j<=n;j++)
			{
				if(j==m || j==n-m+1||i==m||i==n-m+1)
				{
					if((i>=m&&j>=m)&&(i<=n-m+1&&j<=n-m+1))
					{
					  System.out.print("# ");
					}
					else
